...
- MariaDb config:
  - Ssl:
    ...
    - Cert:
      - See NginX section below, but...
      - acme.sh --list
      - Change the `reloadcmd` to also restart `mariadb` like this: `systemctl restart nginx mariadb`
        - grep -rin Le_ReloadCmd ~/.acme.sh/
      - Give access to the key file to `mysql` user:
        - setfacl -m u:mysql:r domain.key
          - `-m` means modify.
...
- Config user:
  ...
  -->
  - `systemctl stop apache2` and `systemctl stop nginx`
  - usermod -s /usr/bin/bash -d /home/www-data www-data
  - `systemctl start apache2` and `systemctl start nginx`
  - mkdir /home/www-data
...
- apt install nginx nginx-extras nginx-doc socat
- acme.sh:
```
cd /tmp
...
- acme.sh --issue --standalone -d DOMAIN.1 -d DOMAIN.2 -d DOMAIN.3
  - OR `--nginx` if port 80 is being used. (There must be a server block with `server_name DOMAIN` for this to work.)
- acme.sh --install-cert -d DOMAIN.1 --fullchain-file /etc/nginx/domain.crt --key-file /etc/nginx/domain.key --reloadcmd 'systemctl force-reload nginx'
...
sub_filter_types *;
sub_filter_once off;
sub_filter "//telegram.org" "//t.tg.domain.ir";
sub_filter "//www.telegram.org" "//t.tg.domain.ir";
...
server {
        ...
        server_name staging.domain-2.ir;

        return 301 https://staging.domain.ir$request_uri;
}
